...
static inline __m512i dequantize_64x4_2_64x8(const uint8_t* rsi) {
    ...
    const __m512i int8s = _mm512_castsi256_si512(int4s);
    const __m512i mask = _mm512_set1_epi8(0xF);
    return _mm512_and_si512(
        _mm512_inserti32x8(int8s, _mm256_srli_epi16(int4s, 4), 1),
        mask
    );
}
#endif
...
static void quantize_row_q8_0(const float * restrict x, void * restrict vy, int k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        maxAbs = _mm256_max_ps( maxAbs, _mm256_andnot_ps( signBit, v4 ) );
        maxAbs = _mm256_max_ps( maxAbs, _mm256_andnot_ps( signBit, v5 ) );
        maxAbs = _mm256_max_ps( maxAbs, _mm256_andnot_ps( signBit, v6 ) );
        maxAbs = _mm256_max_ps( maxAbs, _mm256_andnot_ps( signBit, v7 ) );
        ...
        v4 = _mm256_mul_ps( v4, mul );
        v5 = _mm256_mul_ps( v5, mul );
        v6 = _mm256_mul_ps( v6, mul );
        v7 = _mm256_mul_ps( v7, mul );
        ...
        _mm256_storeu_si256((__m256i *)(y[i].qs + 32), i4);
    }
#else
    // scalar
...
static void quantize_row_q8_1(const float * restrict x, void * restrict vy, int k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].s = d * _mm512_reduce_add_epi32(_mm512_add_epi32(_mm512_add_epi32(i0, i1), _mm512_add_epi32(i2, i3)));
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].s = d * hsum_i32_8(_mm256_add_epi32(_mm256_add_epi32(_mm256_add_epi32(i0, i1), _mm256_add_epi32(i2, i3)),
            _mm256_add_epi32(_mm256_add_epi32(i4, i5), _mm256_add_epi32(i6, i7))));
    ...
    *s = _mm512_reduce_add_ps(acc);
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int j = 0; j < QK4_0 / 32; ++j) {
            ...
            const __m256i bx_off = _mm256_sub_epi8( bx, off );
            const __m256i xy_q = _mm256_dpbssd_epi32(zeros, bx_off, by);
#elif defined(__AVXVNNI__)
            __m256i xy_q = _mm256_dpbusd_epi32(zeros, bx, by);
            const __m256i q_off = _mm256_dpbusd_epi32(zeros, off, by);
            xy_q = _mm256_sub_epi32(xy_q, q_off);
#else
            const __m256i bx_off = _mm256_sub_epi8( bx, off );
            // Get absolute values of x vectors
            const __m256i ax = _mm256_sign_epi8(bx_off, bx_off);
            // Sign the values of the y vectors
            const __m256i sy = _mm256_sign_epi8(by, bx_off);
...
static void ggml_vec_dot_q4_1_q8_1(const int n, float * restrict s, const void * restrict vx, const void * restrict vy) {
    ...
    *s = vaddvq_f32(sumv0) + vaddvq_f32(sumv1) + summs;
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        // xy_q[0] :i32 = sum( ((bx[i] * d0) + m0) * by[i] * d1 )
        // sum(bx[i] * by[i]) * d0 * d1 + sum(by[i]) * d1 * m0
        // sum(bx[i] * by[i]) * d0 * d1 + summs
#ifdef __AVX512VNNI__
        const __m512i xy = _mm512_dpbusd_epi32(zeros, bx, by);
#else
        const __m512i dot = _mm512_maddubs_epi16(bx, by);
        const __m512i xy = _mm512_madd_epi16(ones, dot);
#endif

        /* Convert to vectore of 8 int32_t to 8 floats */
        const __m512 xy_sum = _mm512_cvtepi32_ps(xy);

        acc = _mm512_fmadd_ps( d, xy_sum, acc );
    }
